...
baseline_mod='rtk' # baseline方法:rtk single

# traj_kuanguanglumian = env_param_pd.loc[(env_param_pd['Type']=='kuanguanglumian')]['tripID'].values.tolist()
# traj_openGZ = env_param_pd.loc[(env_param_pd['Type']=='kuanguanglumian') | (env_param_pd['Type']=='gaosu')]['tripID'].values.tolist()
# traj_canyon = env_param_pd.loc[env_param_pd['Type']=='canyon']['tripID'].values.tolist()
# traj_lijiaoqiao = env_param_pd.loc[env_param_pd['Type']=='lijiaoqiao']['tripID'].values.tolist()
# traj_chengshisenlin = env_param_pd.loc[env_param_pd['Type']=='chengshisenlin']['tripID'].values.tolist()
# traj_kexuecheng = env_param_pd.loc[env_param_pd['Type']=='kexuecheng']['tripID'].values.tolist()
# traj_gaosu = env_param_pd.loc[env_param_pd['Type']=='gaosu']['tripID'].values.tolist()

triptype = 'canyon'
trajdata_range_canyon = [0, 10]
traj_type_target_train = [0,1]
postraj_num = 40
# Sensitivity analysis of noise
noisedB = 0 # add noise to observations
noise_sample = 5000 # 采集噪声样本量
noise_levels = [None,0,10,20,30,40]
noise_phi = []
...
class ActionLearner_TD:
    def __init__(self, actionRandomness, decay, stepSize, preTrainTimes=None, maxIter=None, gamma=None, lossMode=None,
        ...
        self.basic.weightReconstruct = scipy.io.loadmat(filename + 'weightReconstruct.mat')['weightReconstruct'].item()
        ...
        if dataset == 'TDurbancanyon':
            self.max_step = 5000
            self.trajdata_range = trajdata_range_canyon
            self.tripIDlist = traj_canyon
            self.env = GPSPosition_discrete_LOS_TDecef(trajdata_range_canyon, traj_type_target_train, triptype, action_scale, discrete_actionspace,
                                                    reward_setting, trajdata_sort, baseline_mod, postraj_num)
    ...
    def learn(self, times):
        ...
        rl_distance = recording_results_ecef_TD(data_truth_dic,self.trajdata_range,self.tripIDlist,self.logdirname,baseline_mod,
                                           traj_record=False)

        with open(self.outputFile + f'_diserror={rl_distance}.txt', 'w') as file:
    ...
    def play_sarsa(self, train=False, render=False):
        ...
        while True:
            ...
            #     noise_phi[idx].append(z)
            #     if self.cnt > noise_sample:
            #         with open(self.outputFile + '_noise.pkl', 'wb') as file:
