...
class RecurrentPPO(OnPolicyAlgorithm):
    ...
    def __init__(
        ...
        self.value_optimizer = torch.optim.SGD(self.value_w.parameters(), lr=self.value_step_size)
    ...
    def predict_batch_value(self,obs: th.tensor):
        ...
        for i in range(obs.shape[0]):
            ...
            z = torch.unsqueeze(torch.FloatTensor(z), 0).cuda()
            batch_list.append(z)
        z = torch.stack(batch_list, dim=0)
        prd_value = self.value_w.forward(z)
        return prd_value
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            with th.no_grad():
                ...
                actions, _, log_probs, lstm_states, lstm_hidden_pi, lstm_hidden_vf,\
                    mlp_hidden_pi, mlp_hidden_vf, cat_features = self.policy.forward(obs_tensor, lstm_states, episode_starts)

            # predict value with TLRL
            values = self.predict_value(self._last_obs)
            ...
            for idx, done_ in enumerate(dones):
                if (
                    ...
                    with th.no_grad():
                        ...
                        terminal_value = self.predict_value(terminal_obs)
        ...
        with th.no_grad():
            ...
            values = self.predict_value(new_obs) # 看看这里是否需要用原来的critic来算
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data,lstm_hidden_pi_buffer,lstm_hidden_vf_buffer,mlp_hidden_pi_buffer,mlp_hidden_vf_buffer in self.rollout_buffer.get(self.batch_size):
                ...
                values_pred = self.predict_batch_value(rollout_data.observations)
                value_loss = th.mean(((rollout_data.returns - values_pred) ** 2)[mask])
        ...
        for param_group in self.value_optimizer.param_groups: # value权重更新设置衰减学习率
            param_group['lr'] *= 0.99
