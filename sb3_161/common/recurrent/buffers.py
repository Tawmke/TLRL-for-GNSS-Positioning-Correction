...
#     RNNStates,
# )
from sb3_161.common.recurrent.type_aliases import (
    RecurrentDictRolloutBufferSamples,
    RecurrentRolloutBufferSamples,
    RNNStates,
)
...
class RecurrentRolloutBuffer(RolloutBuffer):
    ...
    def add(self, *args, lstm_states: RNNStates, **kwargs) -> None:
        ...
        # add mlp hidden state rollout buffer 20230328
        if args[6]==1:
            self.lstm_hidden_pi = args[7]
            self.lstm_hidden_vf = args[8]
            self.mlp_hidden_pi = args[9]
            self.mlp_hidden_vf = args[10]
        elif args[6]==2:
            self.lstm_hidden_pi = th.stack((self.lstm_hidden_pi, args[7]),0)
            self.lstm_hidden_vf = th.stack((self.lstm_hidden_vf, args[8]),0)
            self.mlp_hidden_pi = th.stack((self.mlp_hidden_pi, args[9]),0)
            self.mlp_hidden_vf = th.stack((self.mlp_hidden_vf, args[10]),0)
        else:
            self.lstm_hidden_pi = th.cat((self.lstm_hidden_pi, args[7].unsqueeze(1)),dim=0)
            self.lstm_hidden_vf = th.cat((self.lstm_hidden_vf, args[8].unsqueeze(1)),dim=0)
            self.mlp_hidden_pi = th.cat((self.mlp_hidden_pi, args[9].unsqueeze(1)),dim=0)
            self.mlp_hidden_vf = th.cat((self.mlp_hidden_vf, args[10].unsqueeze(1)),dim=0)

        # self.lstm_hidden_pi[self.pos] = np.array(args[7].cpu().numpy())
        # self.lstm_hidden_vf[self.pos] = np.array(args[8].cpu().numpy())
        # self.mlp_hidden_pi[self.pos] = np.array(args[9].cpu().numpy())
        # self.mlp_hidden_vf[self.pos] = np.array(args[10].cpu().numpy())

        # super().add(*args, **kwargs)
        args1=args[:6]
        super().add(*args1, **kwargs)
    ...
    def _get_samples(
        ...
        ),self.lstm_hidden_pi,self.lstm_hidden_vf,self.mlp_hidden_pi,self.mlp_hidden_vf
...
class RecurrentDictRolloutBuffer(DictRolloutBuffer):
    ...
    def reset(self):
        ...
        self.hidden_states_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)
        self.cell_states_vf = np.zeros(self.hidden_state_shape, dtype=np.float32)
        # lstm_hidden_shape=(self.hidden_state_shape[0],self.hidden_state_shape[2],self.hidden_state_shape[3])
        # self.lstm_hidden_pi = np.zeros(lstm_hidden_shape, dtype=np.float32)
        # self.lstm_hidden_vf = np.zeros(lstm_hidden_shape, dtype=np.float32)
        # mlp_hidden_shape = (self.hidden_state_shape[0], self.hidden_state_shape[2], 64)
        # self.mlp_hidden_pi = np.zeros(mlp_hidden_shape, dtype=np.float32)
        # self.mlp_hidden_vf = np.zeros(mlp_hidden_shape, dtype=np.float32)
    ...
    def _get_samples(
        ...
        # add hidden states for sparse representation 20230327
        # lstm output:  hidden_states_pi, hidden_states_vf (batch_size, seq, feature_size)
        #               cell_states_pi, cell_states_vf (batch_size, seq, feature_size)
        # all lstm states have all-zeros in [0]
