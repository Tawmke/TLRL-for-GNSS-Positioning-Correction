...
from sb3_161.common.policies import ActorCriticPolicy, SPActorCriticPolicy
...
class GNNspRecurrentActorCriticPolicy(SPActorCriticPolicy):
    """
    ...
    def SKL_sparsity_absreslu(self, hidden, beta):
        # Stack the hidden states and calculate the mean along the first dimension
        mean_hidden_states = th.mean(th.abs(hidden), dim=0)

        # Use the th.relu function to create a differentiable mask
        masked_hidden_states = th.relu(mean_hidden_states - beta) + beta

        # Calculate the sparsity loss using the masked hidden states
        sparsity_loss = beta / masked_hidden_states - th.log(beta / masked_hidden_states) - 1

        # Sum the sparsity loss values
        return th.sum(sparsity_loss)
    ...
    def forward(
    ...
    ) -> Tuple[th.Tensor, th.Tensor, th.Tensor, RNNStates, th.Tensor, th.Tensor, th.Tensor, th.Tensor]:
        """
        ...
        """
        # Preprocess the observation if needed # canbe changed to ac seperated feature extractor
        features = self.extract_features(obs)
        # latent_pi, latent_vf = self.mlp_extractor(features)
        #20230602
        features_pi = self.extract_features_pi(obs)
        latent_pi, lstm_states_pi = self._process_sequence(features_pi, lstm_states.pi, episode_starts, self.lstm_actor)
        ...
        latent_pi1 = self.mlp_extractor.forward_actor(latent_pi)
        latent_vf1 = self.mlp_extractor.forward_critic(latent_vf)

        # Evaluate the values for the given observations
        values = self.value_net(latent_vf1)
        distribution = self._get_action_dist_from_latent(latent_pi1)
        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        # add hidden state return for SR  20230327
        # latent_pi, latent_vf (RNN hidden)
        # latent_pi1, latent_vf1 (MLP hidden)
        return actions, values, log_prob, RNNStates(lstm_states_pi, lstm_states_vf), latent_pi, latent_vf, latent_pi1, latent_vf1, features
    ...
    def get_distribution(
        ...
        #20230602
        features_pi = self.extract_features_pi(obs)
        latent_pi, lstm_states = self._process_sequence(features_pi, lstm_states, episode_starts, self.lstm_actor)
    ...
    def evaluate_actions(
        ...
        # latent_pi, latent_vf (RNN hidden)
        # latent_pi1, latent_vf1 (MLP hidden)
        return values, log_prob, distribution.entropy(), latent_pi, latent_vf, latent_pi1, latent_vf1
...
class GNNspRecurrentActorCriticPolicy_DirA(SPActorCriticPolicy):
    """
    Recurrent policy class for actor-critic algorithms (has both policy and value prediction).
    To be used with A2C, PPO and the likes.
    It assumes that both the actor and the critic LSTM
...
class GNNRecurrentActorCriticPolicy(ActorCriticPolicy):
    """
    Recurrent policy class for actor-critic algorithms (has both policy and value prediction).
    To be used with A2C, PPO and the likes.
    It assumes that both the actor and the critic LSTM
...
class TransformerRecurrentActorCriticPolicy(ActorCriticPolicy):
    """
    Recurrent policy class for actor-critic algorithms (has both policy and value prediction).
    To be used with A2C, PPO and the likes.
    It assumes that both the actor and the critic LSTM
    ...
    def __init__(
        ...
        self.transformer_actor = nn.TransformerEncoderLayer(self.features_dim, nhead=2, dim_feedforward=2*self.features_dim, dropout=0.0)
        self.transformer_critic = nn.TransformerEncoderLayer(self.features_dim, nhead=2, dim_feedforward=2*self.features_dim, dropout=0.0)
        # For the predict() method, to initialize hidden states
        # (n_lstm_layers, batch_size, lstm_hidden_size)
        self.lstm_hidden_state_shape = (n_lstm_layers, 1, lstm_hidden_size)
        self.critic = None
    ...
    def forward(
        ...
        features = self.extract_features(obs)
        # latent_pi, latent_vf = self.mlp_extractor(features)
        features1 = self.transformer_actor(features)
        features2 =self.transformer_critic(features)
        latent_pi, lstm_states_pi = self._process_sequence(features1, lstm_states.pi, episode_starts, self.lstm_actor)
        if self.lstm_critic is not None:
            latent_vf, lstm_states_vf = self._process_sequence(features2, lstm_states.vf, episode_starts, self.lstm_critic)
        elif self.shared_lstm:
            # Re-use LSTM features but do not backpropagate
            latent_vf = latent_pi.detach()
            lstm_states_vf = (lstm_states_pi[0].detach(), lstm_states_pi[1].detach())
            ...
            # Critic only has a feedforward network
            latent_vf = self.critic(features2)
            lstm_states_vf = lstm_states_pi
    ...
    def get_distribution(
        ...
        features = self.extract_features(obs)
        features1 = self.transformer_actor(features)
        latent_pi, lstm_states = self._process_sequence(features1, lstm_states, episode_starts, self.lstm_actor)
        latent_pi = self.mlp_extractor.forward_actor(latent_pi)
        return self._get_action_dist_from_latent(latent_pi), lstm_states
    ...
    def predict_values(
        ...
        features2 =self.transformer_critic(features)
        if self.lstm_critic is not None:
            latent_vf, lstm_states_vf = self._process_sequence(features2, lstm_states, episode_starts, self.lstm_critic)
        elif self.shared_lstm:
            # Use LSTM from the actor
            latent_pi, _ = self._process_sequence(features2, lstm_states, episode_starts, self.lstm_actor)
            latent_vf = latent_pi.detach()
        else:
            latent_vf = self.critic(features2)
    ...
    def evaluate_actions(
        ...
        features1 = self.transformer_actor(features)
        features2 =self.transformer_critic(features)
        latent_pi, _ = self._process_sequence(features1, lstm_states.pi, episode_starts, self.lstm_actor)

        if self.lstm_critic is not None:
            latent_vf, _ = self._process_sequence(features2, lstm_states.vf, episode_starts, self.lstm_critic)
        elif self.shared_lstm:
            latent_vf = latent_pi.detach()
        else:
            latent_vf = self.critic(features2)
...
class SepExtRecurrentActorCriticPolicy(ActorCriticPolicy):
    """
    Recurrent policy class for actor-critic algorithms (has both policy and value prediction).
    To be used with A2C, PPO and the likes.
    It assumes that both the actor and the critic LSTM
